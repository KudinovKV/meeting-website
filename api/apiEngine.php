    ...
    function callApiFunction() 
    {
        ...
        if (file_exists($apiName . '.php')) 
        {
            ...
            try {
                ...
                $response = APIConstants::$RESPONSE;
                $jsonParams = $this->apiFunctionParams;
                if ($this->apiFunctionParams) 
                {
                   /* if ($functionName == "getphoto")
                    {//Для возможности возврата не JSON, а бинарных данных таких как zip, png и др. контетнта 
                        return $apiClass->$functionName($jsonParams);//Вызываем метод в API
                    }
                    else*/
                    {
                        $resultFunctionCall->$response = $apiClass->$functionName($jsonParams);//Вызываем метод в API который вернет JSON обект
                    }
                } 
                else 
                {
